...
static int	process_all_heredocs(t_cmd **cmds, int cmd_count, char **env,
				t_shell *shell_ctx)
{
	int	h;
	int	f;
...
static void	setup_pipes_in_child(int i, int cmd_count, int prev_pipe,
				int pipes[2])
{
	if (i > 0)
	{
...
static void	execute_child(t_cmd **cmds, int i, int cmd_count,
...
static int	handle_pipeline_iteration(t_cmd **cmds, int i, int cmd_count, int *prev_pipe,
									char **env, t_env *env_list, t_shell *shell_ctx,
									pid_t *last_pid, int pipes[2])
{
	pid_t	pid;

	if (i < cmd_count - 1 && pipe(pipes) == -1)
		return (handle_pipeline_error(cmds, cmd_count, "minishell: pipe", shell_ctx));
	pid = fork();
	if (pid == 0)
	...
	else if (pid > 0)
	{
		...
		*last_pid = pid;
		...
		return (handle_pipeline_error(cmds, cmd_count, "minishell: fork", shell_ctx));
	return (0);
}

static void	fork_and_execute_pipeline(t_cmd **cmds, int cmd_count, char **env,
									  t_env *env_list, t_shell *shell_ctx,
									  pid_t *last_pid, int *prev_pipe)
{
	int	pipes[2];
	int	i;
	...
	*last_pid = -1;
	...
	{
		if (handle_pipeline_iteration(cmds, i, cmd_count, prev_pipe,
									  env, env_list, shell_ctx, last_pid, pipes))
			break ;
	}
}

void	execute_pipeline(t_cmd **cmds, int cmd_count, char **env,
	t_env *env_list, t_shell *shell_ctx)
{
	int		prev_pipe;
	pid_t	last_pid;
	...
	if (!process_all_heredocs(cmds, cmd_count, env, shell_ctx))
		return ;
	ignore_sigint();
	fork_and_execute_pipeline(cmds, cmd_count, env, env_list, shell_ctx, &last_pid, &prev_pipe);
	if (prev_pipe != -1)
		close(prev_pipe);
