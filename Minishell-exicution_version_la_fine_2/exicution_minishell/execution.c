...
/*   Updated: 2025/07/06 20:00:27 by maskour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int cmd_process(t_cmd *cmd, char **env, t_shell *shell)
{
    ...
    status = redirections(cmd, env, shell);
     if (status == 1)
        exit (1);
     else if (status != 0) // Other errors
        exit (status);  // error in redirection
    ...
    free(cmd_path);
    return 0; // success (should never reach here if execve works)
}
...
static void execute_single_command(t_cmd **cmd, char **envp, t_shell *shell_ctx)
{
    ...
    if (id == 0)
    {
        ...
        int ret = cmd_process(*cmd, envp, shell_ctx);
        exit(ret);
    }
    else if (id > 0)
    {
